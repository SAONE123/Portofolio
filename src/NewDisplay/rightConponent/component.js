export const Logos = [
  {
    key: 1,
    img: "https://img.icons8.com/plasticine/100/css3.png",
  },
  {
    key: 2,
    img: "https://img.icons8.com/color/48/html-5--v1.png",
  },
  {
    key: 3,
    img: "https://img.icons8.com/external-tanah-basah-basic-outline-tanah-basah/100/external-react-social-media-tanah-basah-basic-outline-tanah-basah.png",
  },
  {
    key: 4,
    img: "https://img.icons8.com/color/48/nextjs.png",
  },
  {
    key: 5,
    img: "https://img.icons8.com/ios/50/php-logo.png",
  },
  {
    key: 6,
    img: "https://img.icons8.com/fluency/48/my-sql.png",
  },
  {
    key: 7,
    img: "https://img.icons8.com/color/48/mongodb.png",
  },
  {
    key: 8,
    img: "https://img.icons8.com/color/100/mysql-logo.png",
  },
  {
    key: 9,
    img: "https://img.icons8.com/nolan/64/javascript-logo.png",
  },
  {
    key: 10,
    img: "https://img.icons8.com/color/48/nodejs.png",
  },
  {
    key: 11,
    img: "https://img.icons8.com/color/48/tailwindcss.png",
  },
];
export const Iconscollection = [
  {
    key: 1,
    img: "/logos/github.svg",
    command: "https://github.com/SAONE123",
  },
  {
    key: 2,
    img: "/logos/linkedin.svg",
    command: "https://www.linkedin.com/in/alfian-setyobudi-710b521b7/",
  },
  {
    key: 3,
    img: "/logos/instagram.svg",
    command: "ttps://www.instagram.com/alfian_1920/",
  },
  {
    key: 4,
    img: "/logos/mail.svg",
    command: "mailto:titanalfian112@gmail.com",
  },
];
export const Projectdata = [
  {
    title: "Photographic",
    url: "https://shuff.netlify.app/",
    img: "/project_1.png",
    deskripsi:
      "Shuff Photo Studio represents the ideas and passion shown through our quality and conceptual works. We have a strong vision in creating the best visuals, enabling your brand to provide a special experience to everyone. We provide documentation of photography and videography services with professional photographers + videographers and complete equipment so that photos and videos can provide the best results for your company. Contact us to get offers for photography + videography services in Jakarta at affordable prices.",
  },
  {
    title: "project figma",
    url: "https://fp03.netlify.app/",
    img: "/project_2.png",
    deskripsi: "converting figma to web",
  },
  {
    title: "Animekun",
    url: "http://animekun.great-site.net/",
    img: "/project_3.png",
    deskripsi: "anime web with database mysql from scraping",
  },
  {
    title: "SAONE",
    url: "",
    img: "/kotaimg.jpg",
    deskripsi: "liburan dulu lah",
  },
];
export const AboutDesk = [
  {
    ket: "SMA PERMATA INSANI ISLAMIC SCHOOL",
    list: "Education",
    img: "./about/sma.png",
    deskripsi:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Nulla architecto veniam perspiciatis quis ipsam, adipisci vel quasi! Fugit, impedit suscipit. Quia ducimus id ad nesciunt!",
  },
  {
    ket: "UNIVERSITY OF SULTAN AGENG TIRTAYASA",
    list: "Education",
    img: "./about/kuliah.png",
    deskripsi:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Blanditiis quaerat a quo, neque doloremque aliquid itaque dolorum non, iusto ut aspernatur molestiae aut soluta ea explicabo, veritatis rem error minus.",
  },
  {
    ket: "Student Council",
    list: "Organisasi",
    img: "./about/osis.jpeg",
    deskripsi:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Nulla architecto veniam perspiciatis quis ipsam, adipisci vel quasi! Fugit, impedit suscipit. Quia ducimus id ad nesciunt!",
  },
  {
    ket: "Student Asosiation",
    list: "Organisasi",
    img: "./about/hima.jpg",
    deskripsi:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Nulla architecto veniam perspiciatis quis ipsam, adipisci vel quasi! Fugit, impedit suscipit. Quia ducimus id ad nesciunt!",
  },
  {
    ket: "Proggramming",
    list: "Hobby",
    img: "./about/prog.gif",
    deskripsi:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Nulla architecto veniam perspiciatis quis ipsam, adipisci vel quasi! Fugit, impedit suscipit. Quia ducimus id ad nesciunt!",
  },
  {
    ket: "Gaming",
    list: "Hobby",
    img: "./about/games.gif",
    deskripsi:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Nulla architecto veniam perspiciatis quis ipsam, adipisci vel quasi! Fugit, impedit suscipit. Quia ducimus id ad nesciunt!",
  },
  {
    ket: "Fishing",
    list: "Hobby",
    img: "./about/mancing.gif",
    deskripsi:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Nulla architecto veniam perspiciatis quis ipsam, adipisci vel quasi! Fugit, impedit suscipit. Quia ducimus id ad nesciunt!",
  },
];
